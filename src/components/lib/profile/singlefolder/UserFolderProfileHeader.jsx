...
class UserFolderProfileHeader extends Component {
  ...
  onKeyDownForFolderNameInput(e) {
    if (e.key === 'Enter' && e.shiftKey === false) {
      ...
      this.context.actions.call('folders.editFolderName', {
        _id: this.props.folder._id,
        newName: this.state.folderNameValue
      }, (error, result) => {
        if (!error) {
          this.setState({isEditingFolderName: false});
        }
      });
    }
  }
  ...
  onKeyDownForFolderDescriptionInput(e) {
    if (e.key === 'Enter' && e.shiftKey === false) {
      ...
      this.context.actions.call('folders.editFolderDescription', {
        _id: this.props.folder._id,
        newDesctiption: this.state.folderDescriptionValue
      }, (error, result) => {
        if (!error) {
          this.setState({isEditingFolderDescription: false});
        }
      });
    }
  }
        ...
        <span className="editable-text--button"
              ...
              <svg width="19" height="19" viewBox="0 0 19 19">
                  <g fill="#B5B5B5">
                      ...
                      <path
                        d="M16.70122,0 C16.3097077,0 15.9188703,0.149175518 15.6205109,0.447189053 L9.22854018,6.8393175 L9.22854018,9 L11.3892835,9 L17.7812542,2.60787156 C18.3776355,2.01116948 18.3776355,1.04389112 17.7812542,0.447189053 C17.4828947,0.149175518 17.0920574,0 16.70122,0 Z"/>
                  </g>
              </svg>
          </span>
      );
